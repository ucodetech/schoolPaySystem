<?php
 ...
 class General
 {
  ...
  public function totalCount2(){
    $sql = "SELECT * FROM generated_code WHERE deleted = 0 ";
    $count =  $this->_db->query($sql);
    if ($count->count()) {
      return $count->count();
    }else{
  ...
  public function getResult($jambNo){
    $count =  $this->_db->get('results', array('student_jambno', '=', $jambNo));
    if ($count->count()) {
      return $count->first();
    }else{
      return false;
...
public function getLevel()
     {
         $sch = $this->_db->get('payment_level', array('deleted', '=', 0));
         if ($sch->count()){
             return $sch->results();
         }else {
             return false;
...
public function updateStudents($studid,$fields = array()){
      $this->_db->update('students','stu_id', $studid,$fields);
      return true;
...
public function checkGenerated($userid,$term)
    {
        $check = $this->_db->query("SELECT * FROM generated_code WHERE student_id = '$userid' AND term = '$term' AND deleted = 0 ");
        if ($check->count()){
            return true;
        }else{
            return false;
...
public function grabFQA()
{
  ...
  if ($dat->count()) {
      $rows = $dat->results();
      $output = '';
      $output .='<table class="table" id="showfaq">
...
public function grabFQA2($faq_ide)
{
  ...
  if ($get->count()){
    ...
    public function grabResult($resultid)
    {
        $get = $this->_db->get('results', array('id', '=', $resultid));
        if ($get->count()){
            return $get->first();
        }else{
            return false;
...
public function getTerm()
    {
        $get = $this->_db->get('payment_term', array('deleted', '=', 0));
        if ($get->count()){
            return $get->results();
        }else{
    return false;
...
public function getFee($stu_id)
    {
        $get = $this->_db->get('payment_status', array('student_id', '=', $stu_id));
        if ($get->count()){
            return true;
        }else{
    return false;
...
public function getGeneratedCode($user_id)
    {
//        $get = $this->_db->get('generated_code', array('student_id', '=', $user_id));
        $get = $this->_db->query("SELECT * FROM generated_code WHERE student_id = '$user_id' AND deleted = 0 ");
        if ($get->count()){
            return $get->results();
        }else{
            return false;
    ...
    public function getAmount($userlevel)
    {
        $get = $this->_db->get('fee_amount', array('level', '=', $userlevel));
        if ($get->count()){
            return $get->first();
        }else{
            return false;
...
public function grabComplainInput()
{
  ...
  if ($dat->count()) {
    ...
    public function checkJamb($jamb)
{
  $dat = $this->_db->get('results', array('student_jambno', '=', $jamb));
  if ($dat->count()) {
      return true;
  }else{
    return false;
