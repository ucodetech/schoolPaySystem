...
// fetch admins
public function grabCodes()
{
	$check = $this->_db->get('generated_code', array('deleted', '=', 0));
	if ($check->count()) {
		$data =  $check->results();
		$output = '';

		$output .='<table class="table table-hover" id="totCodes">
				<thead class="text-warning">
					<th>#</th>
					<th>Student Name</th>
				...
				<tbody>
					...
					$x = 0;
		foreach ($data as $ad) {
            $user = new User($ad->student_id);
			$x = $x+1;
			$output .='
			<tr>
				<td>'.$x.'</td>
				<td>'.$user->data()->stud_fname.'</td>
				<td>'.$user->data()->stud_regNo.'</td>
				<td>'.$ad->retrieval_code.'</td>
				<td>'.$ad->amount.'</td>
				<td>'.$ad->term.'</td>
				<td>'.$ad->level.'</td>
				<td>'.pretty_dates($ad->date_generated).'</td>
				<td>'.pretty_dates($ad->payment_date).'</td>
				<td>'.$ad->status.'</td>
				<td>
                 <a href="#" d-id="'.$ad->id.'"  class="text-danger  trashResultIcon" title="Trash Result"><i class="fa fa-trash fa-lg"></i></a>&nbsp;
                </td>
			</tr>
			';
...
public function grabPayments()
    {
        $check = $this->_db->get('transactions', array('deleted', '=', 0));
        if ($check->count()) {
            $data =  $check->results();
            $output = '';

            $output .='<table class="table table-hover" id="showPayments">
				<thead class="text-warning">
					<th>#</th>
					<th>Student Reg No</th>
				    <th>Student Email</th>
			...
			<tr>
				...
				<td>'.$ad->retrieval_code.'</td>
				<td>'.$ad->amount.'</td>
				<td>'.$ad->transaction_ref.'</td>
				<td>'.pretty_dates($ad->datePayed).'</td>
				<td>'.$ad->payment_status.'</td>
				<td>
                 <a href="#" v-id="'.$ad->id.'"  class="text-info  verifyPaymentIcon" title="Verify Payment"><i class="fa fa-check fa-lg"></i></a>&nbsp;
                </td>
			</tr>
			';
...
{
	$sql = "SELECT * FROM students WHERE level = '$advisor_level'";
	$data = $this->_db->query($sql);
	if ($data->count()) {
		return   $data->results();
