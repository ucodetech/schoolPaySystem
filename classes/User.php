...
//login
public function login($regno = null, $password = null)
{
    $show = new Show();
  $user = $this->findUser($regno);
...
//        $uniqueid = $this->data()->stud_unique_id;
        ...
        if (password_verify($password, $this->data()->stud_password)) {
            ...
            $sql = "UPDATE students SET stud_last_login = NOW() WHERE stud_regNo = '$regno' ";
            $this->_db->query($sql);

            return true;
//            process this down lines if u are sending otp on login
